...
def get_simulations(simengine_host, virl_user, virl_password):
    u = simengine_host + "/simengine/rest/list"
    r = requests.get(u, auth=(virl_user, virl_password))
    return r.json()["simulations"]

def get_nodes(simulation, simengine_host, virl_user, virl_password):
    u = simengine_host + "/simengine/rest/nodes/{}".format(simulation)
    r = requests.get(u, auth=(virl_user, virl_password))
    return r.json()[simulation]

def stop_nodes(simulation, nodes, simengine_host, virl_user, virl_password):
...
def start_nodes(simulation, nodes, simengine_host, virl_user, virl_password):
...
def test_node_state(simulation, target_state, simengine_host, virl_user, virl_password, test_nodes=None):
    nodes = get_nodes(simulation, simengine_host, virl_user, virl_password)
    if test_nodes == None:
        test_nodes = nodes
...
def get_node_console(simulation, node, simengine_host, virl_user, virl_password):
...
def kill_simulation(simulation, simengine_host, virl_user, virl_password):
    u = simengine_host + "/simengine/rest/stop/{}".format(simulation)
    r = requests.get(u, auth=(virl_user, virl_password))
    return "{} {}".format(r.status_code, r.text)

def launch_simulation(simulation_name, simulation_data, simengine_host, virl_user, virl_password):
